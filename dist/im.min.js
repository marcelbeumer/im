...
(function(window,environment){var VERSION="post-1.2.1-dev";var prevNS=window.im,callee=arguments.callee,constructors=environment?environment.constructors:[];var im=function(selector,context){return new Chain(selector,context);};im.version=VERSION;if(environment){environment.im=im;}else{window.im=im;}
im.addConstructor=function(fn){constructors.push(fn);};im.env=function(win,doc){win=win||window;var o={constructors:constructors},l=constructors.length,x;callee(win,o);for(x=0;x<l;x++){constructors[x](o.im,win,doc||win.document);}
return o.im;};im.noConflict=function(){ns.im=prevNS;return im;};im.browser=(function(){try{var userAgent=navigator.userAgent.toLowerCase();}catch(e){var userAgent='';}
...
return firstObj;};var rtrim=/^(\s|\u00A0)+|(\s|\u00A0)+$/g;im.trim=function(str){return(str||"").replace(rtrim,"");};im.proxy=function(fn,scope){return function(){return fn.apply(scope,arguments);};};im.chains={};var Chain=function(selector,context){this.length=0;if(!selector){}else if(selector.nodeType){this.length=1;this[0]=selector;}else if(im.is_string(selector)){if(selector.match('<.*>')){if(!im.create)throw new Error("Chain: no create implementation loaded");var el=im.create(selector);if(!el)return this;this.length=1;this[0]=el;}else{if(!im.selectNodes)throw new Error("Chain: no selectNodes implementation loaded");im.merge(this,im.selectNodes(selector,context));}}else if(im.is_function(selector)){if(!im.onready)throw new Error("Chain: no onready implementation loaded");im.onready(selector);}else if(selector.length!==undefined){im.merge(this,selector);}else{this.length=1;this[0]=selector;}
return this;};Chain.prototype=im.chains;var pushStack=function(obj,selector){var n=im(selector);n.__prev=obj;return n;};im.chains.reset=function(selector){return pushStack(this,selector);};im.chains.end=function(){return this.__prev||im();};im.chains.exec=function(fn){fn.apply(this);return this;};im.chains.array=function(){var a=[];im.merge(a,this);return a;};im.chains.item=function(number){number=number||0;if(this.length>number&&this[number])return this.reset(this[number]);return this.reset();};im.chains.el=function(number){number=number||0;if(this.length>number&&this[number])return this[number];};im.each=function(obj,callback){var length=obj.length;var isObject=length===undefined||im.is_function(obj);if(isObject){var name;for(name in obj){if(callback.call(obj[name],name,obj[name])===false)break;}}else{var i=0;for(var value=obj[0];i<length&&callback.call(value,i,value)!==false;value=obj[++i]){}}};im.chains.each=function(callback){im.each(this,callback);return this;};})(window,false);im.add_constructor(function(im,window,document){im.getAncestors=function(element,resultArr){var a=resultArr||[];var p=element.parentNode;while(p){a.push(p);p=p.parentNode;}
...
return size;};im.width=function(el,options){return getSize(el,'Width',options);};im.chains.width=function(options){if(this.length>0)return im.width(this[0],options);};im.height=function(el,options){return getSize(el,'Height',options);};im.chains.height=function(options){if(this.length>0)return im.height(this[0],options);};});im.add_constructor(function(im,window,document){var AXISALL=0;var AXISCHILD=1;var bitCache={};var resultCache={};var parseSelector=function(selector){var joins=selector.split(',');var jl=joins.length;while(jl--){joins[jl]=joins[jl].split(' ');}
...
if(this.length>1)results=filterUniqueNodes(results);return this.reset(results);};im.chains.filter=function(selector,invert){var results=[];if(im.is_function(selector)){for(var x=0;x<this.length;x++){var m=selector.apply(this[x],[x]);if((!invert&&m)||(invert&&!m))results.push(this[x]);}}else{var s=im.selectNodes(selector||'*');var l=s.length;for(var x=0;x<this.length;x++){for(var y=0;y<l;y++){var m=s[y]===this[x];if((!invert&&m)||(invert&&!m)){results.push(this[x]);break;}}}}
return this.reset(results);};});im.add_constructor(function(im,window,document){var STOP_CLASSNAME='scan-stop';var bitCache={};var registry={};var rtrim=/^(\s|\u00A0)+|(\s|\u00A0)+$/g;var trim=function(str){return(str||"").replace(rtrim,"");};var hasClass=function(node,className){var cn=node.className;var r=new RegExp("(^|\\s)"+className+"(\\s|$)");return r.test(cn);};var hasClasses=function(className,classes,regex){var l=classes.length;while(l--){if(!regex[l].test(className))return false;}
return true;};var makeHasClassesRegexes=function(classes){var r=[];var l=classes.length;while(l--){r.push(new RegExp("(^|\\s)"+classes[l]+"(\\s|$)"));}
...
return parsed;};im.scan=function(){var l=arguments.length,t=[],nodes=[],str=Object.prototype.toString;while(l--){var a=arguments[l];if(str.call(a)=='[object String]'){t=t.concat(registry[a]||[]);}else if(str.call(a)=='[object Array]'){t=t.concat(a);}else if(a.nodeType){nodes.push(a);}}
if(t.length==0)return;t=parseTriggers(t);if(nodes.length==0)nodes.push(document.body);var nl=nodes.length;while(nl--){walk(nodes[nl],t);}};im.scan.register=function(){var type,selector,fn,l=arguments.length;if(l<3)throw new Error('scan.register takes at least three parameters');selector=arguments[l-2];fn=arguments[l-1];for(var x=0;x<l-2;x++){var a=arguments[x];var r=registry[a]=registry[a]||[];r.push([selector,fn]);}};im.scan.registry=registry;});(function(){window.im=im.env();}());