...
return this;};Chain.prototype=im.chains;var pushStack=function(obj,selector){var n=im(selector);n.__prev=obj;return n;};im.chains.reset=function(selector){return pushStack(this,selector);};im.chains.end=function(){return this.__prev||im();};im.chains.exec=function(fn){fn.apply(this);return this;};im.chains.array=function(){var a=[];im.merge(a,this);return a;};im.chains.item=function(number){number=number||0;if(this.length>number&&this[number])return this.reset(this[number]);return this.reset();};im.chains.el=function(number){number=number||0;if(this.length>number&&this[number])return this[number];};im.each=function(obj,callback){var length=obj.length;var isObject=length===undefined||im.isFunction(obj);if(isObject){var name;for(name in obj){if(callback.call(obj[name],name,obj[name])===false)break;}}else{var i=0;for(var value=obj[0];i<length&&callback.call(value,i,value)!==false;value=obj[++i]){}}};im.chains.each=function(callback){im.each(this,callback);return this;};})(window,false);im.addConstructor(function(im,window,document){im.getAncestors=function(element,resultArr){var a=resultArr||[];var p=element.parentNode;while(p){a.push(p);p=p.parentNode;}
...
return size;};im.width=function(el,options){return getSize(el,'Width',options);};im.chains.width=function(options){if(this.length>0)return im.width(this[0],options);};im.height=function(el,options){return getSize(el,'Height',options);};im.chains.height=function(options){if(this.length>0)return im.height(this[0],options);};});im.addConstructor(function(im,window,document){var AXISALL=0;var AXISCHILD=1;var bitCache={};var resultCache={};var parseSelector=function(selector){var joins=selector.split(',');var jl=joins.length;while(jl--){joins[jl]=joins[jl].split(' ');}
