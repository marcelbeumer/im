...
(function(window,environment){var version="post-1.2.1-dev",prev_im=environment?environment.prev_im:window.im,callee=arguments.callee,constructors=environment?environment.constructors:{},im;im=function(selector,context){return new Chain(selector,context);};im.version=version;if(environment){environment.im=im;}else{window.im=im;}
im.register=function(name,fn){constructors[name]=fn;};im.env=function(win,keep_prev_im){win=win?(win.contentWindow||win):window;var e={constructors:constructors,prev_im:keep_prev_im?im.noConflict(true):null},name;callee(win,e);for(name in constructors){constructors[name](e.im,win,win.document);}
...
return this;};Chain.prototype=im.chains;var push_stack=function(obj,selector){var n=im(selector);n.__prev=obj;return n;};im.chains.reset=function(selector){return push_stack(this,selector);};im.chains.end=function(){return this.__prev||im();};im.chains.exec=function(fn){fn.apply(this);return this;};im.chains.array=function(){var a=[];im.merge(a,this);return a;};im.chains.item=function(number){number=number||0;if(this.length>number&&this[number])return this.reset(this[number]);return this.reset();};im.chains.el=function(number){number=number||0;if(this.length>number&&this[number])return this[number];};im.each=function(obj,callback){var length=obj.length;var isObject=length===undefined||im.isFunction(obj);if(isObject){var name;for(name in obj){if(callback.call(obj[name],name,obj[name])===false)break;}}else{var i=0;for(var value=obj[0];i<length&&callback.call(value,i,value)!==false;value=obj[++i]){}}};im.chains.each=function(callback){im.each(this,callback);return this;};})(window,false);im.register('dom',function(im,window,document){im.getAncestors=function(element,resultArr){var a=resultArr||[];var p=element.parentNode;while(p){a.push(p);p=p.parentNode;}
