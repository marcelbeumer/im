...
(function(ns){var VERSION="post-1.1.2-dev";var prevNS=ns.im;var im=ns.im=function(selector,context){return new Chain(selector,context);};im.version=VERSION;im.noConflict=function(){ns.im=prevNS;return im;};im.browser=(function(){try{var userAgent=navigator.userAgent.toLowerCase();}catch(e){var userAgent='';}
...
ea[x]=e.o=e.h=null;}
im.data(element,'eventHandlers',ed);};if(im.browser.msie){window.attachEvent('onunload',function(){var data=im.data();for(var x=0;x<data.length;x++){try{var elem=im.getElementByUUID(x);if(!data[x]||!elem||!data[x].eventHandlers)continue;for(var name in data[x].eventHandlers){try{im.unbind(elem,name);}catch(e){}}}catch(e){}}});}
