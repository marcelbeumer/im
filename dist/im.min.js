...
return firstObj;};var rtrim=/^(\s|\u00A0)+|(\s|\u00A0)+$/g;im.trim=function(str){return(str||"").replace(rtrim,"");};im.chains={};var Chain=function(selector,context){this.length=0;if(!selector){}else if(im.isString(selector)){if(selector.match('<.*>')){if(!im.create)throw new Error("Chain: no create implementation loaded");var el=im.create(selector);if(!el)return this;this.length=1;this[0]=el;}else{if(!im.selectNodes)throw new Error("Chain: no selectNodes implementation loaded");im.merge(this,im.selectNodes(selector,context));}}else if(im.isFunction(selector)){if(!im.onready)throw new Error("Chain: no onready implementation loaded");im.onready(selector);}else if(selector.length!==undefined){im.merge(this,selector);}else{this.length=1;this[0]=selector;}
return this;};Chain.prototype=im.chains;var pushStack=function(obj,selector){var n=im(selector);n.__prev=obj;return n;};im.chains.reset=function(selector){return pushStack(this,selector);};im.chains.end=function(){return this.__prev||im();};im.chains.array=function(){var a=[];im.merge(a,this);return a;};im.chains.item=function(number){number=number||0;if(this.length>number&&this[number])return this.reset(this[number]);return this.reset();};im.chains.el=function(number){number=number||0;if(this.length>number&&this[number])return this[number];};im.each=function(obj,callback){var length=obj.length;var isObject=length===undefined||im.isFunction(obj);if(isObject){var name;for(name in obj){if(callback.call(obj[name],name,obj[name])===false)break;}}else{var i=0;for(var value=obj[0];i<length&&callback.call(value,i,value)!==false;value=obj[++i]){}}};im.chains.each=function(callback){im.each(this,callback);return this;};})(window);(function(im){im.getAncestors=function(element,resultArr){var a=resultArr||[];var p=element.parentNode;while(p){a.push(p);p=p.parentNode;}
return a;};im.getAncestorOrSelf=function(nodeOrClassName,element){var match=false;if(nodeOrClassName.indexOf('.')==0){if(im.hasClass(element.className,nodeOrClassName))match=true;}else{if(nodeOrClassName==element.nodeName.toLowerCase())match=true;}
...
element.style.display='none';return element;};im.chains.hide=function(){for(var x=0;x<this.length;x++)im.hide(this[x]);return this;};im.remove=function(element){element.parentNode.removeChild(element);};im.chains.remove=function(){for(var x=0;x<this.length;x++)im.remove(this[x]);return this;};im.appendTo=function(element,appendTo){im(appendTo).el(0).appendChild(element);};im.chains.appendTo=function(appendTo){a=im(appendTo);for(var x=0;x<this.length;x++)im.appendTo(this[x],a);return this;};im.append=function(element,append){var e=im(append);for(var x=0;x<e.length;x++)element.appendChild(e[x]);};im.chains.append=function(append){for(var x=0;x<this.length;x++)im.append(this[x],append);return this;};im.html=function(element,value){if(value||value=='')element.innerHTML=value;else return im.trim(element.innerHTML);};im.chains.html=function(value){if(value||value==''){for(var x=0;x<this.length;x++)im.html(this[x],value);return this;}else{if(this.length>0)return im.html(this[0]);}};im.create=function(html){var div=document.createElement('div');div.innerHTML=im.trim(html);return div.firstChild;};})(window.im||window);(function(im){var checkPreventDefault=function(e,returnValue){if(returnValue!==false)return;if(e&&e.preventDefault){e.preventDefault();}else{if(window.event)window.event.returnValue=false;}
return false;};var fixEventObject=function(e){if(!e.target)e.target=e.srcElement||document;if(e.target.nodeType===3)event.target=event.target.parentNode;};var validateMouseEnterLeave=function(element,e,eventName){var related=e.relatedTarget?e.relatedTarget:(eventName=="mouseleave"?e.toElement:e.fromElement);if(related&&(related==element||im.isAncestorOf(element,related)))return false;return true;};var getBindHandler=function(element,eventName,handler){if(eventName=="mouseenter"||eventName=="mouseleave"){return function(e){fixEventObject(e);if(validateMouseEnterLeave(this===window?e.target:this,e,eventName)){return checkPreventDefault(e,handler.apply(element,[e]));}};}else{return function(e){fixEventObject(e);return checkPreventDefault(e,handler.apply(element,[e]));};}};var translateEventName=function(eventName){if(eventName=="mouseleave"){eventName="mouseout";}else if(eventName=="mouseenter"){eventName="mouseover";}
return eventName;};im.bind=function(element,eventName,handler){var h=getBindHandler(element,eventName,handler);eventName=translateEventName(eventName);var ed=im.data(element,'eventHandlers')||{};ed[eventName]=ed[eventName]||[];ed[eventName].push({h:h,o:handler});im.data(element,'eventHandlers',ed);if(window.addEventListener){element.addEventListener(eventName,h,true);}else{element.attachEvent('on'+eventName,h);}};im.chains.bind=function(eventName,handler){for(var x=0;x<this.length;x++)im.bind(this[x],eventName,handler);return this;};im.unbind=function(element,eventName,handler){eventName=translateEventName(eventName);var ed=im.data(element,'eventHandlers')||{};var ea=ed[eventName]||[];for(var x=0;x<ea.length;x++){var e=ea[x];if(e&&e.h&&(!handler||e.o===handler)){if(window.removeEventListener){element.removeEventListener(eventName,e.h,true);}else{element.detachEvent('on'+eventName,e.h);}}
ea[x]=e.o=e.h=null;}
im.data(element,'eventHandlers',ed);};if(im.browser.msie){window.attachEvent('onunload',function(){var data=im.data();for(var x=0;x<data.length;x++){var elem=im.getElementByUUID(x);if(!data[x]||!elem||!data[x].eventHandlers)continue;for(var name in data[x].eventHandlers){try{im.unbind(elem,name);}catch(e){}}}});}
im.chains.unbind=function(eventName,handler){for(var x=0;x<this.length;x++)im.unbind(this[x],eventName,handler);return this;};im.live=function(element,selector,eventName,handler){im.bind(element,translateEventName(eventName),function(e){var el;var t=im(e.target);if(t.filter(selector).length>0){el=e.target;}else{var inv=t.parents(selector);if(inv.length>0)el=inv[0];}
if(el&&handler){if(eventName=="mouseenter"||eventName=="mouseleave"){if(validateMouseEnterLeave(el,e,eventName)===true)return handler.apply(el,[e]);}else{return handler.apply(el,[e]);}}});};im.chains.live=function(selector,eventName,handler){for(var x=0;x<this.length;x++)im.live(this[x],selector,eventName,handler);return this;};})(window.im||window);(function(im){var readyHandlers=[];var readyBound=false;var isReady=false;var ready=function(){if(isReady)return;isReady=true;var l=readyHandlers.length;for(var x=0;x<l;x++){readyHandlers[x]();}};var bindReady=function(){if(readyBound)return;readyBound=true;if(document.addEventListener&&!im.browser.opera){document.addEventListener("DOMContentLoaded",ready,false);}
...
return size;};im.width=function(el,options){return getSize(el,'Width',options);};im.chains.width=function(options){if(this.length>0)return im.width(this[0],options);};im.height=function(el,options){return getSize(el,'Height',options);};im.chains.height=function(options){if(this.length>0)return im.height(this[0],options);};})(window.im||window);(function(im){var AXISALL=0;var AXISCHILD=1;var bitCache={};var parseSelector=function(selector){var joins=selector.split(',');var jl=joins.length;while(jl--){joins[jl]=joins[jl].split(' ');}
return joins;};var hasClasses=function(className,classes,regex){var l=classes.length;while(l--){if(!regex[l].test(className))return false;}
...
return r;};var parseSelectionBit=function(bit){var parts=bit.split('.');var nodeName=parts.shift();var remains=!!(parts.length>0);return{nodeName:nodeName==''?'*':nodeName.toUpperCase(),classNames:remains?parts:null,classNamesRegexes:remains?makeHasClassesRegexes(parts):null};};var getContexts=function(currentContexts,axis,selection){var result=[];var nodeName=selection.nodeName;var classNames=selection.classNames;var regex=selection.classNamesRegexes;var _hasClasses=hasClasses;var cl=currentContexts.length;if(axis==AXISCHILD){for(var x=0;x<cl;x++){var context=currentContexts[x];var n=context.firstChild;for(;n;n=n.nextSibling){if(n.nodeType==1&&(nodeName=='*'||nodeName==n.nodeName)){if(classNames){var c=n.className;if(!c)continue;if(_hasClasses(c,classNames,regex))result.push(n);}else{result.push(n);}}}}}else{for(var x=0;x<cl;x++){var context=currentContexts[x];var nodes=context.getElementsByTagName(nodeName);var l=nodes.length;for(var y=0;y<l;y++){if(classNames){var n=nodes[y],c=n.className;if(!c)continue;if(_hasClasses(c,classNames,regex))result.push(n);}else{result.push(nodes[y]);}}}}
...
return nodes;};im.selectNodes=function(selector,context,resultArr){context=context||document;if(!context)throw new Error('im.selectNodes - could not get context, are you are the DOM is ready?');var parsed=parseSelector(selector);var results;var axis=AXISALL;var pl=parsed.length;for(var x=0;x<pl;x++){var currentContexts=context.length?[].concat(context):[context];var psel=parsed[x];var bl=psel.length;for(var y=0;y<bl;y++){if(currentContexts.length==0)break;var bit=psel[y];if(bit==''){}else if(bit=='>'){axis=AXISCHILD;}else if(/^#/.test(bit)){var el=document.getElementById(bit.substring(1,bit.length));currentContexts=el?[el]:[];axis=AXISALL;}else{if(!bitCache[bit])bitCache[bit]=parseSelectionBit(bit);currentContexts=getContexts(currentContexts,axis,bitCache[bit]);axis=AXISALL;}}
results=(x==0)?currentContexts:results.concat(currentContexts);results=filterUniqueNodes(results);};if(resultArr){results=filterUniqueNodes(results.concat(resultArr));resultArr.splice(0,resultArr.length);var l=results.length;for(var x=0;x<l;x++)resultArr.push(results[x]);}
