...
(function(window,document,environment){var version="1.2.2-rc1",prev_im=environment?environment.prev_im:window.im,callee=arguments.callee,constructors=environment?environment.constructors:{},im;im=function(selector,context){return new Chain(selector,context);};im.version=version;if(environment){environment.im=im;}else{window.im=im;}
im.register=function(name,fn){constructors[name]=fn;if(environment)fn(im,window,document);};im.env=function(win,keep_prev_im){win=win?(win.contentWindow||win):window;var e={constructors:constructors,prev_im:keep_prev_im?im.noConflict(true):null},doc=(im.browser.msie?win.Document:win.contentDocument)||win.document;name;callee(win,doc,e);for(name in constructors){constructors[name](e.im,win,doc);}
return e.im;};im.noConflict=function(no_restore){if(no_restore===true)return prev_im;if(prev_im===null)return false;window.im=prev_im;return im;};im.browser=(function(){try{var userAgent=navigator.userAgent.toLowerCase();}catch(e){var userAgent='';}
...
return this;};Chain.prototype=im.chains;var push_stack=function(obj,selector){var n=im(selector);n.__prev=obj;return n;};im.chains.reset=function(selector){return push_stack(this,selector);};im.chains.end=function(){return this.__prev||im();};im.chains.exec=function(fn){fn.apply(this);return this;};im.chains.array=function(){var a=[];im.merge(a,this);return a;};im.chains.item=function(number){number=number||0;if(this.length>number&&this[number])return this.reset(this[number]);return this.reset();};im.chains.el=function(number){number=number||0;if(this.length>number&&this[number])return this[number];};im.each=function(obj,callback){var length=obj.length;var isObject=length===undefined||im.isFunction(obj);if(isObject){var name;for(name in obj){if(callback.call(obj[name],name,obj[name])===false)break;}}else{var i=0;for(var value=obj[0];i<length&&callback.call(value,i,value)!==false;value=obj[++i]){}}};im.chains.each=function(callback){im.each(this,callback);return this;};})(window,document,false);im.register('dom',function(im,window,document){im.getAncestors=function(element,resultArr){var a=resultArr||[];var p=element.parentNode;while(p){a.push(p);p=p.parentNode;}
...
return size;};im.width=function(el,options){return getSize(el,'Width',options);};im.chains.width=function(options){if(this.length>0)return im.width(this[0],options);};im.height=function(el,options){return getSize(el,'Height',options);};im.chains.height=function(options){if(this.length>0)return im.height(this[0],options);};});im.register('selector',function(im,window,document){var AXISALL=0;var AXISCHILD=1;var bitCache={};var resultCache={};var parseSelector=function(selector){var joins=selector.split(',');var jl=joins.length;while(jl--){joins[jl]=joins[jl].split(' ');}
...
if(t.length==0)return;t=parseTriggers(t);if(nodes.length==0)nodes.push(document.body);var nl=nodes.length;while(nl--){walk(nodes[nl],t);}};im.scan.register=function(){var type,selector,fn,l=arguments.length;if(l<3)throw new Error('scan.register takes at least three parameters');selector=arguments[l-2];fn=arguments[l-1];for(var x=0;x<l-2;x++){var a=arguments[x];var r=registry[a]=registry[a]||[];r.push([selector,fn]);}};im.scan.registry=registry;});(function(){window.im=im.env(window,true);}());