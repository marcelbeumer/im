...
(function(ns){var VERSION="post-1.2.1-dev";var prevNS=ns.im;var im=ns.im=function(selector,context){return new Chain(selector,context);};im.version=VERSION;im.noConflict=function(){ns.im=prevNS;return im;};im.browser=(function(){try{var userAgent=navigator.userAgent.toLowerCase();}catch(e){var userAgent='';}
return{version:(userAgent.match(/.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/)||[])[1],safari:/webkit/.test(userAgent),opera:/opera/.test(userAgent),msie:/msie/.test(userAgent)&&!(/opera/.test(userAgent)),mozilla:/mozilla/.test(userAgent)&&!(/(compatible|webkit)/.test(userAgent))};})();im.setWindow=function(windowObj){im.__win=windowObj;im.__doc=windowObj.document;};im.setWindow(window);im.isFunction=function(obj){return Object.prototype.toString.call(obj)==="[object Function]";};im.isArray=function(obj){return Object.prototype.toString.call(obj)==="[object Array]";};im.isString=function(obj){return Object.prototype.toString.call(obj)==="[object String]";};im.isObject=function(obj){return Object.prototype.toString.call(obj)==="[object Object]";};im.merge=function(first,second){var i=first.length,j=0;if(typeof second.length==="number"){for(var l=second.length;j<l;j++){first[i++]=second[j];}}else{while(second[j]!==undefined){first[i++]=second[j++];}}
...
if(toplevel)(function(){try{im.__doc.documentElement.doScroll("left");}catch(e){setTimeout(arguments.callee,0);return;}
callback();})();};impl.validateElement=function(element){return element===document;};impl.bindRealEvent=function(callback){if(im.__doc.addEventListener){_callback=callback;im.__doc.addEventListener("DOMContentLoaded",_callback,false);}else if(im.__win.attachEvent){_callback=function(){if(im.__doc.readyState==="complete")callback();};im.__doc.attachEvent("onreadystatechange",_callback);if(im.browser.msie)explorerScrollCheck(callback);}
im.bind(im.__win,'load',callback);};impl.unbindRealEvent=function(callback){if(im.__doc.removeEventListener){im.__doc.removeEventListener("DOMContentLoaded",_callback,false);}else if(im.__win.detachEvent){im.__doc.detachEvent("onreadystatechange",_callback);}};return uniqueEventTypeFactory(impl);})();im.bind.types.load=(function(){var impl={};impl.validateElement=function(element){return element===im.__win;};impl.bindRealEvent=function(callback){if(im.__win.addEventListener){im.__win.addEventListener("load",callback,false);}else if(im.__win.attachEvent){im.__win.attachEvent("onload",callback);}};impl.unbindRealEvent=function(callback){if(im.__doc.removeEventListener){im.__doc.removeEventListener("load",callback,false);}else if(im.__win.detachEvent){im.__doc.detachEvent("onload",callback);}};impl.beforeRun=function(){im.bind.types.ready.run();};return uniqueEventTypeFactory(impl);})();im.onready=function(handler){im.bind(im.__doc,'ready',handler);};im.onload=function(handler){im.bind(im.__win,'load',handler);};})(window.im);(function(im){var colorTypes=[{re:/^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/,convert:function(bits){return[parseInt(bits[1],10),parseInt(bits[2],10),parseInt(bits[3],10)];}},{re:/^#(\w{2})(\w{2})(\w{2})$/,convert:function(bits){return[parseInt(bits[1],16),parseInt(bits[2],16),parseInt(bits[3],16)];}},{re:/^#(\w{1})(\w{1})(\w{1})$/,convert:function(bits){return[parseInt(bits[1]+bits[1],16),parseInt(bits[2]+bits[2],16),parseInt(bits[3]+bits[3],16)];}}];var colorCache={};var interpolate=function(source,target,pos){return(source+(target-source)*pos).toFixed(3);};var getColor=function(color){var c,l=colorTypes.length;while(l--){var m=colorTypes[l].re.exec(color);if(m){c=colorTypes[l].convert(m);break;}}
...
return scrollbarWidth;};})();var getMargin=function(el,dir){var v=parseInt(im.css(el,'margin'+dir),10);if(isNaN(v))v=0;return v;};var getSize=function(el,axis,options){options=options||{};if(el===window||el===document){var windowSize,documentSize;var opp=axis=='Width'?'Height':'Width',bodyClientOpp=im.__doc.body['client'+opp],bodyScrollOpp=im.__doc.body['scroll'+opp],windowInnerCur=im.__win['inner'+axis],docElClientCur=im.__doc.documentElement['client'+axis],bodyClientCur=im.__doc.body['client'+axis],bodyScrollCur=im.__doc.body['scroll'+axis];if(!im.browser.msie){windowSize=windowInnerCur;var sb=options.includeScrollbar?0:getScrollbarWidth();if(bodyClientOpp<bodyScrollOpp)windowSize=windowSize-sb;}else{windowSize=docElClientCur||bodyClientCur;}
...
return result;};var filterUniqueNodes=function(nodes){var duplicate=false;var sort=(function(){if(im.__doc.documentElement.compareDocumentPosition){return function(a,b){var r=a.compareDocumentPosition(b)&4?-1:a===b?0:1;if(r==0)duplicate=true;return r;};}else if("sourceIndex"in im.__doc.documentElement){return function(a,b){var r=a.sourceIndex-b.sourceIndex;if(r==0)duplicate=true;return r;};}})();nodes.sort(sort);if(duplicate){var l=nodes.length;for(var x=1;x<l;x++){if(nodes[x]==nodes[x-1]){nodes.splice(x--,1);l--;}}}
...
if(t.length==0)return;t=parseTriggers(t);if(nodes.length==0)nodes.push(im.__doc.body);var nl=nodes.length;while(nl--){walk(nodes[nl],t);}};ns.scan.register=function(){var type,selector,fn,l=arguments.length;if(l<3)throw new Error('scan.register takes at least three parameters');selector=arguments[l-2];fn=arguments[l-1];for(var x=0;x<l-2;x++){var a=arguments[x];var r=registry[a]=registry[a]||[];r.push([selector,fn]);}};ns.scan.registry=registry;})(window.im||window);